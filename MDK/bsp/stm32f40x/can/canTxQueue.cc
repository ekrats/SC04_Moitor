...
#define canTxRamSize          (sizeof(CAN_TX_DATA_RAM) * canTxQueueSize)

TxQueue::TxQueue(void)
{
	
}

void TxQueue::QueueInit(void)
{
	this->DataFIFO = (CAN_TX_DATA_RAM *)rt_malloc(canTxRamSize);
	this->FifoRamPtr = 0;
...
void TxQueue::Push_Msg(uint8_t port, CanTxMsg* msg)
{
	if (this->Register != FIFO_FULL_MASK)
	{
		...
		//------------------------------------------------------------------------
		this->FifoRamPtr++;
		if (this->FifoRamPtr >= canTxQueueSize)
			this->FifoRamPtr = 0;
		//------------------------------------------------------------------------
		// 弹出信号量, 进行数据操作
		//------------------------------------------------------------------------
...
void TxQueue::pop_Q(uint32_t msg)
{
...
void TxQueue::Init(void)
{
	this->QStart 	  = this->QueueRam;
	this->QEnd 	 	  = &this->QueueRam[canTxQueueSize-1];
	this->QSize  	  = canTxQueueSize;
	this->QEntriesMax = 0;
...
* Return         : None
*******************************************************************************/
uint16_t TxQueue::GetFifoReg(void)
{
...
* Return         : None
*******************************************************************************/
void TxQueue::SetFifoReg(uint16_t val)
{
	this->Register = val;
}
